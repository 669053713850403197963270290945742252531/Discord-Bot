...
The pipeline has a production **semantic-safe** default profile. It validates the Luau AST but keeps the uploaded user program byte-for-byte unchanged unless an experimental source transformation is explicitly enabled in configuration. Local renaming, control-flow rewriting, MBA literal rewriting, string-pool rewriting, comment removal, and synthetic source noise remain opt-in experimental transformations because syntax-only rewriting cannot prove preservation for every form of Luau (callbacks, metatables, overloaded behavior, custom environments, executor APIs, and debug-sensitive code). The complete program payload is then protected by the polymorphic VM layer.

The VM layer is intentionally a source VM rather than a claim to reproduce Luau's native bytecode. Every build gets a new runtime surface: physical instruction order is shuffled, the eight-field record schema is permuted, virtual opcode values are randomized, program-counter edges are masked, and the dispatcher is emitted as one of several state-machine shapes. **Intense VM Structure** is an automatic obfuscation factor on every build; it adds multiple opaque relay stages after every real payload block. The control-flow layer adds reachable opaque branch islands plus unreachable graph islands; both sides of reachable decoys reconverge so they can perturb the VM's private shadow state without changing the reconstructed program. Dead-code generation creates build-specific handler bodies for inactive opcodes and interleaves those blocks through the generated runtime. Multi-layer payload protection combines the primary byte transform, a separately derived secondary byte transform, word packing, per-word lane masks, and a keyed permutation.

**VM Compression** uses a lossless LZ-style codec before payload encryption. It is most useful for larger or repetitive source files; small or already-high-entropy sources can receive too little compression to offset the added decompression/runtime scaffolding.

The runtime also performs distributed anti-tamper checks: a schema seal at startup, a whole-record metadata seal, per-record guards during dispatch, periodic sampled-record checks, per-chunk keyed hashes, output-size checks, and a final keyed payload hash before invoking the native Luau loader. These features deliberately operate **around** the user's source in the production profile. The original Luau payload remains byte-for-byte unchanged until it is reconstructed at runtime, which avoids the callback/property/metatable regressions caused by syntax-only source rewriting. The result is a much less stable generated surface across builds without pretending that runtime observation can be made mathematically impossible: an attacker controlling the execution environment can still instrument code after plaintext reconstruction.
