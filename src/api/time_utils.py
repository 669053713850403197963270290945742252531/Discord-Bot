...
def hwid_reset_cooldown_remaining(last_reset: Optional[str]) -> Optional[timedelta]:
    """Return the remaining 7-day HWID reset cooldown, or None when available."""
    if not last_reset:
        return None
    text = str(last_reset).strip()
    try:
    ...
    remaining = config.RESET_HWID_COOLDOWN - (datetime.now(timezone.utc) - reset_at.astimezone(timezone.utc))
