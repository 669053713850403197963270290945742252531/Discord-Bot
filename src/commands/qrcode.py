...
class QRCode(commands.Cog):
    ...
    @app_commands.describe(
        ...
        color="Foreground color -- hex (#FF0000) or name (red). Type for presets. Ignored if rainbow is on.",
        ...
        eye_ball_color="Override color for eye balls (centers) only -- hex or name. Default: follows body color/gradient.",
        logo="An image for the center of the code. Auto-raises error_correction to High if not set explicitly.",
        logo_shape="Shape to crop the logo (and its background plate) to. Default Circle.",
        logo_size=f"Logo size as % of the full image, {LOGO_SIZE_MIN}-{LOGO_SIZE_MAX}. Default {DEFAULT_LOGO_SIZE_PERCENT}. Shrunk further near finders.",
